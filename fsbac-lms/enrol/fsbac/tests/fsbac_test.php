...
global $CFG;
require_once($CFG->dirroot.'/enrol/fsbac/lib.php');
require_once($CFG->dirroot.'/enrol/fsbac/locallib.php');
 ...
 */
class fsbac_test extends \advanced_testcase {

    public function test_basics() {
        $this->assertTrue(enrol_is_enabled('fsbac'));
        $plugin = enrol_get_plugin('fsbac');
        $this->assertInstanceOf('enrol_fsbac_plugin', $plugin);
        $this->assertEquals(1, get_config('enrol_fsbac', 'defaultenrol'));
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, get_config('enrol_fsbac', 'expiredaction'));
    }
    ...
    public function test_sync_nothing() {
        ...
        $fsbacplugin->sync($trace, null);
        $fsbacplugin->sync($trace, $SITE->id);
    }
    ...
    public function test_longtimnosee() {
        ...
        $this->assertEquals(3, $DB->count_records('enrol', array('enrol'=>'fsbac')));
        $instance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'fsbac'), '*', MUST_EXIST);
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'fsbac'), '*', MUST_EXIST);
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'fsbac'), '*', MUST_EXIST);
        $id = $fsbacplugin->add_instance($course3, array('status'=>ENROL_INSTANCE_ENABLED, 'roleid'=>$teacherrole->id));
        ...
        $fsbacplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $fsbacplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $fsbacplugin->enrol_user($instance1, $user3->id, $studentrole->id);
        ...
        $fsbacplugin->enrol_user($instance3, $user1->id, $studentrole->id);
        $fsbacplugin->enrol_user($instance3, $user2->id, $studentrole->id);
        $fsbacplugin->enrol_user($instance3, $user3->id, $studentrole->id);
        $fsbacplugin->enrol_user($instance3b, $user1->id, $teacherrole->id);
        $fsbacplugin->enrol_user($instance3b, $user4->id, $teacherrole->id);
        ...
        $fsbacplugin->sync($trace, $course2->id);
        $output = $trace->get_buffer();
        $trace->reset_buffer();
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertStringContainsString('No expired enrol_fsbac enrolments detected', $output);
    ...
    public function test_expired() {
        ...
        $fsbacplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $fsbacplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $fsbacplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $fsbacplugin->enrol_user($instance3, $user1->id, $studentrole->id, 0, 0);
        $fsbacplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now-60*60);
        $fsbacplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now+60*60);
        $fsbacplugin->enrol_user($instance3b, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $fsbacplugin->enrol_user($instance3b, $user4->id, $teacherrole->id);
        ...
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, $fsbacplugin->get_config('expiredaction'));
        $fsbacplugin->sync($trace, null);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));


        $fsbacplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $fsbacplugin->sync($trace, $course2->id);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));
        ...
        $fsbacplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
    ...
    public function test_send_expiry_notifications() {
        ...
        /** @var $fsbacplugin enrol_fsbac_plugin */
        $fsbacplugin = enrol_get_plugin('fsbac');
        ...
        $fsbacplugin->set_config('expirynotifylast', $now - 60*60*24);
        $fsbacplugin->set_config('expirynotifyhour', 0);
        ...
        $this->assertEquals(4, $DB->count_records('enrol', array('enrol'=>'fsbac')));
        ...
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'fsbac'), '*', MUST_EXIST);
        ...
        $fsbacplugin->enrol_user($instance1, $user1->id, $studentrole->id, 0, $now + 60*60*24*1, ENROL_USER_SUSPENDED); // Suspended users are not notified.
        $fsbacplugin->enrol_user($instance1, $user2->id, $studentrole->id, 0, $now + 60*60*24*5);                       // Above threshold are not notified.
        $fsbacplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now + 60*60*24*3 + 60*60);               // Less than one day after threshold - should be notified.
        $fsbacplugin->enrol_user($instance1, $user4->id, $studentrole->id, 0, $now + 60*60*24*4 - 60*3);                // Less than one day after threshold - should be notified.
        $fsbacplugin->enrol_user($instance1, $user5->id, $studentrole->id, 0, $now + 60*60);                            // Should have been already notified.
        $fsbacplugin->enrol_user($instance1, $user6->id, $studentrole->id, 0, $now - 60);                               // Already expired.
        ...
        $fsbacplugin->enrol_user($instance2, $user1->id, $studentrole->id);
        $fsbacplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60*3);                // Above threshold are not notified.
        $fsbacplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $manualplugin->enrol_user($maninstance3, $user1->id, $editingteacherrole->id);
        $fsbacplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);                  // Above threshold are not notified.
        $fsbacplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $manualplugin->enrol_user($maninstance4, $user4->id, $editingteacherrole->id);
        $fsbacplugin->enrol_user($instance4, $user5->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);
        $fsbacplugin->enrol_user($instance4, $user6->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);
        ...
        $fsbacplugin->send_expiry_notifications($trace);
        ...
        $fsbacplugin->set_config('expirynotifylast', time() - 60*60*24);
        $fsbacplugin->set_config('expirynotifyhour', '24');
    ...
    public function test_show_enrolme_link() {
        ...
        $DB->update_record('enrol', $instance1);
        $fsbacplugin->update_status($instance1, ENROL_INSTANCE_ENABLED);
        ...
        $DB->update_record('enrol', $instance3);
        $fsbacplugin->update_status($instance3, ENROL_INSTANCE_DISABLED);

        // New enrolments are not allowed and enrolment instance is disabled.
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'fsbac'), '*', MUST_EXIST);
        $instance4->customint6 = 0;
        $DB->update_record('enrol', $instance4);
        $fsbacplugin->update_status($instance4, ENROL_INSTANCE_DISABLED);

        // Cohort member test.
        $instance5 = $DB->get_record('enrol', array('courseid'=>$course5->id, 'enrol'=>'fsbac'), '*', MUST_EXIST);
        $instance5->customint6 = 1;
        $instance5->customint5 = $cohort1->id;
        $DB->update_record('enrol', $instance1);
        $fsbacplugin->update_status($instance5, ENROL_INSTANCE_ENABLED);

        $id = $fsbacplugin->add_instance($course5, $fsbacplugin->get_instance_defaults());
        $instance6 = $DB->get_record('enrol', array('id'=>$id), '*', MUST_EXIST);
        $instance6->customint6 = 1;
        $instance6->customint5 = $cohort2->id;
        $DB->update_record('enrol', $instance1);
        $fsbacplugin->update_status($instance6, ENROL_INSTANCE_ENABLED);

        // Enrol start date is in future.
        $instance7 = $DB->get_record('enrol', array('courseid'=>$course6->id, 'enrol'=>'fsbac'), '*', MUST_EXIST);
        $instance7->customint6 = 1;
        $instance7->enrolstartdate = time() + 60;
        $DB->update_record('enrol', $instance7);
        $fsbacplugin->update_status($instance7, ENROL_INSTANCE_ENABLED);

        // Enrol start date is in past.
        $instance8 = $DB->get_record('enrol', array('courseid'=>$course7->id, 'enrol'=>'fsbac'), '*', MUST_EXIST);
        $instance8->customint6 = 1;
        $instance8->enrolstartdate = time() - 60;
        $DB->update_record('enrol', $instance8);
        $fsbacplugin->update_status($instance8, ENROL_INSTANCE_ENABLED);

        // Enrol end date is in future.
        $instance9 = $DB->get_record('enrol', array('courseid'=>$course8->id, 'enrol'=>'fsbac'), '*', MUST_EXIST);
        $instance9->customint6 = 1;
        $instance9->enrolenddate = time() + 60;
        $DB->update_record('enrol', $instance9);
        $fsbacplugin->update_status($instance9, ENROL_INSTANCE_ENABLED);

        // Enrol end date is in past.
        $instance10 = $DB->get_record('enrol', array('courseid'=>$course9->id, 'enrol'=>'fsbac'), '*', MUST_EXIST);
        $instance10->customint6 = 1;
        $instance10->enrolenddate = time() - 60;
        $DB->update_record('enrol', $instance10);
        $fsbacplugin->update_status($instance10, ENROL_INSTANCE_ENABLED);

        // Maximum enrolments reached.
        $instance11 = $DB->get_record('enrol', array('courseid'=>$course10->id, 'enrol'=>'fsbac'), '*', MUST_EXIST);
        $instance11->customint6 = 1;
        $instance11->customint3 = 1;
        $DB->update_record('enrol', $instance11);
        $fsbacplugin->update_status($instance11, ENROL_INSTANCE_ENABLED);
        $fsbacplugin->enrol_user($instance11, $user2->id, $studentrole->id);

        // Maximum enrolments not reached.
        $instance12 = $DB->get_record('enrol', array('courseid'=>$course11->id, 'enrol'=>'fsbac'), '*', MUST_EXIST);
        $instance12->customint6 = 1;
        $instance12->customint3 = 1;
        $DB->update_record('enrol', $instance12);
        $fsbacplugin->update_status($instance12, ENROL_INSTANCE_ENABLED);

        $this->setUser($user1);
        $this->assertTrue($fsbacplugin->show_enrolme_link($instance1));
        $this->assertFalse($fsbacplugin->show_enrolme_link($instance2));
        $this->assertFalse($fsbacplugin->show_enrolme_link($instance3));
        $this->assertFalse($fsbacplugin->show_enrolme_link($instance4));
        $this->assertFalse($fsbacplugin->show_enrolme_link($instance7));
        $this->assertTrue($fsbacplugin->show_enrolme_link($instance8));
        $this->assertTrue($fsbacplugin->show_enrolme_link($instance9));
        $this->assertFalse($fsbacplugin->show_enrolme_link($instance10));
        $this->assertFalse($fsbacplugin->show_enrolme_link($instance11));
        $this->assertTrue($fsbacplugin->show_enrolme_link($instance12));

        require_once("$CFG->dirroot/cohort/lib.php");
        cohort_add_member($cohort1->id, $user1->id);

        $this->assertTrue($fsbacplugin->show_enrolme_link($instance5));
        $this->assertFalse($fsbacplugin->show_enrolme_link($instance6));
    }
    ...
    public function test_can_self_enrol() {
        ...
        $expectederrorstring = get_string('canntenrol', 'enrol_fsbac');
        ...
        $DB->update_record('enrol', $instance1);
        $fsbacplugin->update_status($instance1, ENROL_INSTANCE_ENABLED);
        $fsbacplugin->enrol_user($instance1, $user2->id, $editingteacherrole->id);

        $this->setUser($guest);
        $this->assertStringContainsString(get_string('noguestaccess', 'enrol'),
                $fsbacplugin->can_self_enrol($instance1, true));

        $this->setUser($user1);
        $this->assertTrue($fsbacplugin->can_self_enrol($instance1, true));

        // Active enroled user.
        $this->setUser($user2);
        $fsbacplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $this->setUser($user1);
        $this->assertSame($expectederrorstring, $fsbacplugin->can_self_enrol($instance1, true));
    }
    ...
    public function test_is_self_enrol_available() {
        ...
        // New enrolments are allowed and enrolment instance is enabled.
        $instance = $DB->get_record('enrol', ['courseid' => $course->id, 'enrol' => 'fsbac'], '*', MUST_EXIST);
        $instance->customint6 = 1;
        $DB->update_record('enrol', $instance);
        $fsbacplugin->update_status($instance, ENROL_INSTANCE_ENABLED);
        $this->setUser($user1);
        $this->assertTrue($fsbacplugin->is_self_enrol_available($instance));
        $this->setGuestUser();
        $this->assertTrue($fsbacplugin->is_self_enrol_available($instance));

        $canntenrolerror = get_string('canntenrol', 'enrol_fsbac');

        // New enrolments are not allowed, but enrolment instance is enabled.
        $instance->customint6 = 0;
        $DB->update_record('enrol', $instance);
        $this->setUser($user1);
        $this->assertEquals($canntenrolerror, $fsbacplugin->is_self_enrol_available($instance));
        $this->setGuestUser();
        $this->assertEquals($canntenrolerror, $fsbacplugin->is_self_enrol_available($instance));
        ...
        $instance->customint6 = 1;
        $DB->update_record('enrol', $instance);
        $fsbacplugin->update_status($instance, ENROL_INSTANCE_DISABLED);
        $this->setUser($user1);
        $this->assertEquals($canntenrolerror, $fsbacplugin->is_self_enrol_available($instance));
        $this->setGuestUser();
        $this->assertEquals($canntenrolerror, $fsbacplugin->is_self_enrol_available($instance));
        ...
        $DB->update_record('enrol', $instance);
        $error = get_string('canntenrolearly', 'enrol_fsbac', userdate($instance->enrolstartdate));
        $this->setUser($user1);
        $this->assertEquals($error, $fsbacplugin->is_self_enrol_available($instance));
        $this->setGuestUser();
        $this->assertEquals($error, $fsbacplugin->is_self_enrol_available($instance));
        ...
        $instance->enrolenddate = time() - 60;
        $DB->update_record('enrol', $instance);
        $error = get_string('canntenrollate', 'enrol_fsbac', userdate($instance->enrolenddate));
        $this->setUser($user1);
        $this->assertEquals($error, $fsbacplugin->is_self_enrol_available($instance));
        $this->setGuestUser();
        $this->assertEquals($error, $fsbacplugin->is_self_enrol_available($instance));
        ...
        $DB->update_record('enrol', $instance);
        $fsbacplugin->enrol_user($instance, $user2->id, $studentrole->id);
        $error = get_string('maxenrolledreached', 'enrol_fsbac');
        $this->setUser($user1);
        $this->assertEquals($error, $fsbacplugin->is_self_enrol_available($instance));
        $this->setGuestUser();
        $this->assertEquals($error, $fsbacplugin->is_self_enrol_available($instance));
        ...
        $DB->update_record('enrol', $instance);
        $error = get_string('cohortnonmemberinfo', 'enrol_fsbac', $cohort1->name);
        $this->setUser($user1);
        $this->assertStringContainsString($error, $fsbacplugin->is_self_enrol_available($instance));
        $this->setGuestUser();
        $this->assertStringContainsString($error, $fsbacplugin->is_self_enrol_available($instance));
        $this->setUser($user2);
        $this->assertEquals($canntenrolerror, $fsbacplugin->is_self_enrol_available($instance));
    ...
    /**
     * Test enrol_fsbac_check_group_enrolment_key
     */
    public function test_enrol_fsbac_check_group_enrolment_key() {
        global $DB;
        ...
        $result = enrol_fsbac_check_group_enrolment_key($course->id, 'invalidpassword');
        $this->assertFalse($result);

        $result = enrol_fsbac_check_group_enrolment_key($course->id, 'thepassword');
        ...
        $result = enrol_fsbac_check_group_enrolment_key($othercourse->id, 'thepassword');
        $this->assertFalse($result);
    ...
    public function test_get_welcome_email_contact() {
        ...
        // Enable fsbac enrolment plugin and set to send email from course contact.
        $fsbacplugin = enrol_get_plugin('fsbac');
        $instance1 = $DB->get_record('enrol', ['courseid' => $course1->id, 'enrol' => 'fsbac'], '*', MUST_EXIST);
        $instance1->customint6 = 1;
        ...
        $contact = $fsbacplugin->get_welcome_email_contact(ENROL_SEND_EMAIL_FROM_COURSE_CONTACT, $context);
        ...
        $fsbacplugin->enrol_user($instance1, $user1->id, $editingteacherrole->id);
        ...
        $DB->update_record('enrol', $instance1);
        $fsbacplugin->enrol_user($instance1, $user3->id, $managerrole->id);

        // Give manager role holdkey capability.
        assign_capability('enrol/fsbac:holdkey', CAP_ALLOW, $managerrole->id, $context);

        // We should get the manager email contact.
        $contact = $fsbacplugin->get_welcome_email_contact(ENROL_SEND_EMAIL_FROM_KEY_HOLDER, $context);
        ...
        $fsbacplugin->enrol_user($instance1, $user4->id, $managerrole->id);
        $contact = $fsbacplugin->get_welcome_email_contact(ENROL_SEND_EMAIL_FROM_KEY_HOLDER, $context);
        $this->assertEquals($user3->username, $contact->username);
        $this->assertEquals($user3->email, $contact->email);
        ...
        $contact = $fsbacplugin->get_welcome_email_contact(ENROL_SEND_EMAIL_FROM_NOREPLY, $context);
        $this->assertEquals($noreplyuser, $contact);
    }
    ...
    public function test_get_user_enrolment_actions() {
        ...
        // Only enable the fsbac enrol plugin.
