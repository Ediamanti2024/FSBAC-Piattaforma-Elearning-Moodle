 ...
 * @since     Moodle 2.6
 */
class enrol_fsbac_external extends external_api {
    ...
    public static function get_instance_info_parameters() {
        return new external_function_parameters([
            'instanceid' => new external_value(PARAM_INT, 'instance id of fsbac enrolment plugin.'),
        ]);
    }

    /**
     * Return fsbac-enrolment instance information.
     *
     * @param int $instanceid instance id of fsbac enrolment plugin.
    ...
    public static function get_instance_info($instanceid) {
        ...
        // Retrieve fsbac enrolment plugin.
        $enrolplugin = enrol_get_plugin('fsbac');
    ...
    public static function enrol_user_parameters() {
        return new external_function_parameters(
            array(
                ...
                'instanceid' => new external_value(PARAM_INT, 'Instance id of fsbac enrolment plugin.', VALUE_DEFAULT, 0)
            )
        );
    }
    ...
    public static function enrol_user($courseid, $password = '', $instanceid = 0) {
        ...
        // Retrieve the fsbac enrolment plugin.
        $enrol = enrol_get_plugin('fsbac');
        if (empty($enrol)) {
            throw new moodle_exception('canntenrol', 'enrol_fsbac');
        }

        // We can expect multiple fsbac-enrolment instances.
        $instances = array();
        $enrolinstances = enrol_get_instances($course->id, true);
        foreach ($enrolinstances as $courseenrolinstance) {
            if ($courseenrolinstance->enrol == "fsbac") {
                // Instance specified.
            ...
            if ($enrolstatus === true) {
                if ($instance->password and $params['password'] !== $instance->password) {
                    ...
                    if ($instance->customint1) {
                        ...
                        if (!enrol_fsbac_check_group_enrolment_key($course->id, $params['password'])) {
                            $warnings[] = array(
                                'item' => 'instance',
                                'itemid' => $instance->id,
                                'warningcode' => '2',
                                'message' => get_string('passwordinvalid', 'enrol_fsbac')
                            );
                            continue;
                        }
                    } else {
                        if ($enrol->get_config('showhint')) {
                            ...
                            $warnings[] = array(
                                ...
                                'itemid' => $instance->id,
                                'warningcode' => '3',
                                'message' => s(get_string('passwordinvalidhint', 'enrol_fsbac', $hint)) // message is PARAM_TEXT.
                            );
                            continue;
                        } else {
                            $warnings[] = array(
                ...
                $enrol->enrol_fsbac($instance, (object) $data);
                $enrolled = true;
                break;
            } else {
                $warnings[] = array(
