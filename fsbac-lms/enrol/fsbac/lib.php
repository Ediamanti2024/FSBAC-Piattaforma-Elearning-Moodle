 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_fsbac_plugin extends enrol_plugin {
    ...
    public function get_info_icons(array $instances) {
        ...
        foreach ($instances as $instance) {
            if ($this->can_self_enrol($instance, false) !== true) {
                ...
                // would hide fsbac-enrolment icons from guests.
                continue;
            }
        ...
        $icons = array();
        if ($nokey) {
            $icons[] = new pix_icon('withoutkey', get_string('pluginname', 'enrol_fsbac'), 'enrol_fsbac');
        }
        if ($key) {
            $icons[] = new pix_icon('withkey', get_string('pluginname', 'enrol_fsbac'), 'enrol_fsbac');
        }
        return $icons;
    }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/fsbac:config', $context)) {
            return false;
        }
     ...
     */
    public function enrol_fsbac(stdClass $instance, $data = null) {
        global $DB, $USER, $CFG;
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        if (true === $enrolstatus) {
            // This user can fsbac enrol using this instance.
            $form = new enrol_fsbac_enrol_form(null, $instance);
            $instanceid = optional_param('instance', 0, PARAM_INT);
            if ($instance->id == $instanceid) {
                if ($data = $form->get_data()) {
                    $this->enrol_fsbac($instance, $data);
                }
            }
        } else {
            // This user can not fsbac enrol using this instance. Using an empty form to keep
            ...
            $form = new enrol_fsbac_empty_form($url, $data);
        }

        ob_start();
        $form->display();
        $output = ob_get_clean();
        // inizio aggiunta di ARIADNE
        $format = course_get_format($COURSE->id);
        $formatname = get_class($format);
        $renderer = $PAGE->get_renderer($formatname);
        $outputclass = $format->get_output_classname('content');
        $widget = new $outputclass($format);
        $output .= $renderer->render($widget);
        // fine aggiunta di ARIADNE
        return $OUTPUT->box($output);
    }

    /**
     * Checks if user can fsbac enrol.
     *
     * @param stdClass $instance enrolment instance
    ...
    public function can_self_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        if ($checkuserenrolment) {
            if (isguestuser()) {
                // Can not enrol guest.
                // return get_string('noguestaccess', 'enrol') . $OUTPUT->continue_button(get_login_url(), "ciao");
                return $OUTPUT->single_button(get_login_url(), get_string("registertostart", "enrol_fsbac"));
            }
            // Check if user is already enroled.
            if ($DB->get_record('user_enrolments', array('userid' => $USER->id, 'enrolid' => $instance->id))) {
                return get_string('canntenrol', 'enrol_fsbac');
            }
        }

        // Check if fsbac enrolment is available right now for users.
        ...
        if (!has_capability('enrol/fsbac:enrolself', context_course::instance($instance->courseid))) {
            return get_string('canntenrol', 'enrol_fsbac');
        }
    ...
    /**
     * Does this plugin support some way to fsbac enrol?
    ...
    public function is_self_enrol_available(stdClass $instance) {
        ...
        if ($instance->enrolstartdate != 0 and $instance->enrolstartdate > time()) {
            return get_string('canntenrolearly', 'enrol_fsbac', userdate($instance->enrolstartdate));
        }

        if ($instance->enrolenddate != 0 and $instance->enrolenddate < time()) {
            return get_string('canntenrollate', 'enrol_fsbac', userdate($instance->enrolenddate));
        }
        ...
        if ($instance->customint3 > 0) {
            ...
            if ($count >= $instance->customint3) {
                // Bad luck, no more fsbac enrolments here.
                return get_string('maxenrolledreached', 'enrol_fsbac');
            }
        }
        ...
        if ($instance->customint5) {
            ...
            if (!cohort_is_member($instance->customint5, $USER->id)) {
                ...
                return markdown_to_html(get_string('cohortnonmemberinfo', 'enrol_fsbac', $a));
            }
        }
    ...
    public function get_enrol_info(stdClass $instance) {
        ...
        if ($instance->password) {
            ...
            $instanceinfo->requiredparam->enrolpassword = get_string('password', 'enrol_fsbac');
        }
        ...
        if ((true === $instanceinfo->status) && $instance->password) {
            $instanceinfo->wsfunction = 'enrol_fsbac_get_instance_info';
        }
        return $instanceinfo;
    }
    ...
    protected function email_welcome_message($instance, $user) {
        ...
        if (!is_null($instance->customtext1) && trim($instance->customtext1) !== '') {
            ...
            $messagetext = get_string('welcometocoursetext', 'enrol_fsbac', $a);
            $messagehtml = text_to_html($messagetext, null, false, true);
        }

        $subject = get_string('welcometocourse', 'enrol_fsbac', format_string($course->fullname, true, array('context' => $context)));

        $sendoption = $instance->customint4;
        // $contact = $this->get_welcome_email_contact($sendoption, $context);

        // Directly emailing welcome message rather than using messaging.
        email_to_user($user, $emailfrom, $subject, $messagetext, $messagehtml);
    }
    ...
    public function sync(progress_trace $trace, $courseid = null) {
        ...
        $trace->output('Verifying fsbac-enrolments...');
        ...
        // Note: the logic of fsbac enrolment guarantees that user logged in at least once (=== u.lastaccess set)
        ...
        $sql = "SELECT e.*, ue.userid
                  ...
                  JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'fsbac' AND e.customint2 > 0)
        ...
        $trace->output('...user fsbac-enrolment updates finished.');
        $trace->finished();
    ...
    /**
     * Returns the user who is responsible for fsbac enrolments in given instance.
     *
     * Usually it is the first editing teacher - the person with "highest authority"
     * as defined by sort_by_roleassignment_authority() having 'enrol/fsbac:manage'
     * capability.
     *
    ...
    protected function get_enroller($instanceid) {
        ...
        if ($users = get_enrolled_users($context, 'enrol/fsbac:manage')) {
        ...
        if ($merge and $instances = $DB->get_records('enrol', $merge, 'id')) {
            ...
            if (!empty($data->customint5)) {
                if ($step->get_task()->is_samesite()) {
                    ...
                    // Use some id that can not exist in order to prevent fsbac enrolment,
    ...
    public function restore_role_assignment($instance, $roleid, $userid, $contextid) {
        ...
        // we do not use component in manual or fsbac enrol.
    ...
    public function can_delete_instance($instance) {
        $context = context_course::instance($instance->courseid);
        return has_capability('enrol/fsbac:config', $context);
    }
    ...
    public function can_hide_show_instance($instance) {
        ...
        if (!has_capability('enrol/fsbac:config', $context)) {
            return false;
        }
    ...
    /**
     * The fsbac enrollment plugin has several bulk operations that can be performed.
    ...
    public function get_bulk_operations(course_enrolment_manager $manager) {
        ...
        if (has_capability("enrol/fsbac:manage", $context)) {
            $bulkoperations['editselectedusers'] = new enrol_fsbac_editselectedusers_operation($manager, $this);
        }
        if (has_capability("enrol/fsbac:unenrol", $context)) {
            $bulkoperations['deleteselectedusers'] = new enrol_fsbac_deleteselectedusers_operation($manager, $this);
        }
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $context) {
        ...
        $mform->addElement('select', 'status', get_string('status', 'enrol_fsbac'), $options);
        $mform->addHelpButton('status', 'status', 'enrol_fsbac');

        $options = $this->get_newenrols_options();
        $mform->addElement('select', 'customint6', get_string('newenrols', 'enrol_fsbac'), $options);
        $mform->addHelpButton('customint6', 'newenrols', 'enrol_fsbac');
        $mform->disabledIf('customint6', 'status', 'eq', ENROL_INSTANCE_DISABLED);

        $passattribs = array('size' => '20', 'maxlength' => '50');
        $mform->addElement('passwordunmask', 'password', get_string('password', 'enrol_fsbac'), $passattribs);
        $mform->addHelpButton('password', 'password', 'enrol_fsbac');
        ...
        $mform->addElement('select', 'customint1', get_string('groupkey', 'enrol_fsbac'), $options);
        $mform->addHelpButton('customint1', 'groupkey', 'enrol_fsbac');

        $roles = $this->extend_assignable_roles($context, $instance->roleid);
        $mform->addElement('select', 'roleid', get_string('role', 'enrol_fsbac'), $roles);

        $options = array('optional' => true, 'defaultunit' => 86400);
        $mform->addElement('duration', 'enrolperiod', get_string('enrolperiod', 'enrol_fsbac'), $options);
        $mform->addHelpButton('enrolperiod', 'enrolperiod', 'enrol_fsbac');
        ...
        $mform->addElement('date_time_selector', 'enrolstartdate', get_string('enrolstartdate', 'enrol_fsbac'), $options);
        $mform->setDefault('enrolstartdate', 0);
        $mform->addHelpButton('enrolstartdate', 'enrolstartdate', 'enrol_fsbac');

        $options = array('optional' => true);
        $mform->addElement('date_time_selector', 'enrolenddate', get_string('enrolenddate', 'enrol_fsbac'), $options);
        $mform->setDefault('enrolenddate', 0);
        $mform->addHelpButton('enrolenddate', 'enrolenddate', 'enrol_fsbac');

        $options = $this->get_longtimenosee_options();
        $mform->addElement('select', 'customint2', get_string('longtimenosee', 'enrol_fsbac'), $options);
        $mform->addHelpButton('customint2', 'longtimenosee', 'enrol_fsbac');

        $mform->addElement('text', 'customint3', get_string('maxenrolled', 'enrol_fsbac'));
        $mform->addHelpButton('customint3', 'maxenrolled', 'enrol_fsbac');
        ...
        if (count($cohorts) > 1) {
            $mform->addElement('select', 'customint5', get_string('cohortonly', 'enrol_fsbac'), $cohorts);
            $mform->addHelpButton('customint5', 'cohortonly', 'enrol_fsbac');
        } else {
        ...
        $mform->addElement('select', 'customint4', get_string('sendcoursewelcomemessage', 'enrol_fsbac'),
                enrol_send_welcome_email_options());
        $mform->addHelpButton('customint4', 'sendcoursewelcomemessage', 'enrol_fsbac');

        $options = array('cols' => '60', 'rows' => '8');
        $mform->addElement('textarea', 'customtext1', get_string('customwelcomemessage', 'enrol_fsbac'), $options);
        $mform->addHelpButton('customtext1', 'customwelcomemessage', 'enrol_fsbac');
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        if ($data['status'] == ENROL_INSTANCE_ENABLED) {
            if (!empty($data['enrolenddate']) and $data['enrolenddate'] < $data['enrolstartdate']) {
                $errors['enrolenddate'] = get_string('enrolenddaterror', 'enrol_fsbac');
            }
        }
    ...
    /**
     * Gets a list of roles that this user can assign for the course as the default for fsbac-enrolment.
     *
     * @param context $context the context.
     * @param integer $defaultrole the id of the role that is set as the default for fsbac-enrolment
    ...
    public function get_welcome_email_contact($sendoption, $context) {
        ...
        if ($sendoption == ENROL_SEND_EMAIL_FROM_COURSE_CONTACT) {
            ...
            // Send as the first user with enrol/fsbac:holdkey capability assigned in the course.
            list($sort) = users_order_by_sql('u');
            $keyholders = get_users_by_capability($context, 'enrol/fsbac:holdkey', 'u.*', $sort);
            if (!empty($keyholders)) {
 ...
 */
function enrol_fsbac_get_fontawesome_icon_map() {
    return [
        'enrol_fsbac:withkey' => 'fa-key',
        'enrol_fsbac:withoutkey' => 'fa-sign-in',
    ];
}
