...
class set_policy_base_accepted_for_manual_users extends \core\task\scheduled_task {

    public function get_name() {
        return get_string('set_policy_base_accepted_for_manual_users', 'local_crons');
    }
    ...
    public function execute() {
        ...
        $sql = "SELECT tpv.id
                 ...
                 WHERE {$likesummary}";
        $policybase = $DB->get_record_sql($sql, array('summary' => '%id="base"%'));
        if ($policybase) {
            $sql = "SELECT id
                      FROM {user}
                     WHERE deleted = 0
            ...
            if (!empty($notacceptedpolicybaseusers)) {
                foreach ($notacceptedpolicybaseusers as $notacceptedpolicybaseuser) {
                    ...
                    mtrace("L'utente " . $notacceptedpolicybaseuser->id . " ha accettato la policy base " . $policybase->id);
                }
            } else {
                mtrace("La policy base " . $policybase->id . " non è stata accettata da nessun nuovo utente");
            }
        }
