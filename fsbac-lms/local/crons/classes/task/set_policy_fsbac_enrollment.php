...
class set_policy_fsbac_enrollment extends \core\task\scheduled_task {

    public function get_name() {
        return get_string('set_policy_fsbac_enrollment', 'local_crons');
    }
    ...
    public function execute() {
        ...
        $sql = "SELECT tpv.id
                 ...
                 WHERE {$likesummary}";
        $policybase = $DB->get_record_sql($sql, array('summary' => '%id="fsbac_necessario"%'));
        if ($policybase) {
            $sql = "SELECT id
                      FROM {user}
                    ...
                    SELECT userid
                     ...
                     AND id in (select distinct ue.userid from {enrol} e inner join {customfield_data} cd on e.courseid=cd.instanceid 
                            inner join {customfield_field} cf on cd.fieldid=cf.id and cf.shortname='pnrr' and intvalue=0
            ...
            if (!empty($notacceptedpolicybaseusers)) {
                foreach ($notacceptedpolicybaseusers as $notacceptedpolicybaseuser) {
                    ...
                    mtrace("L'utente " . $notacceptedpolicybaseuser->id . " ha accettato la policy fsbac " . $policybase->id);
                }
            } else {
                mtrace("La policy fsbac " . $policybase->id . " non è stata accettata da nessun nuovo utente");
            }
        }
