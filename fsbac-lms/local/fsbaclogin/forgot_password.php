...
require_once($CFG->dirroot . '/local/fsbaclogin/classes/forgot_password_form.php');
require_once($CFG->dirroot . '/local/fsbaclogin/classes/set_password_form.php');

$token = optional_param('token', false, PARAM_ALPHANUM);

$PAGE->set_url('/local/fsbaclogin/forgot_password.php');
...
if (empty($token)) {
    ...
    local_fsbaclogin_processpasswordresetrequest();
} else {
    ...
    if (!$tokeninsession && $_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        redirect($CFG->wwwroot . '/local/fsbaclogin/forgot_password.php');
    } else {
        // Continue with the password reset process.
        local_fsbaclogin_processpasswordset($token);
    }
}
