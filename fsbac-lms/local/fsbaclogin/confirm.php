...
$PAGE->set_url('/local/fsbaclogin/confirm.php');
...
if (!empty($data) || (!empty($p) && !empty($s))) {
    ...
    if ($confirmed == AUTH_CONFIRM_ALREADY) {
        ...
        echo $OUTPUT->single_button(local_fsbaclogin_getreturnurl(), get_string('courses'));
        echo $OUTPUT->box_end();
        echo $OUTPUT->footer();
        exit;
        ...
        $buttonurl = local_fsbaclogin_getreturnurl();
        $logourl = $OUTPUT->get_logo_url("50", "50");
        $confirmlogin = new \local_fsbaclogin\output\confirmlogin($fullusername, $buttonurl, $logourl);
        $renderer = $PAGE->get_renderer('local_fsbaclogin');
        echo $renderer->render($confirmlogin);
