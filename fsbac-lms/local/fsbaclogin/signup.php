...
// $cf_added = optional_param('cf_added', false, PARAM_BOOL);
var_dump($cf_added);
var_dump($cf);
...
if (!empty($cf)) {
    if (!$cf_added) {
        // if (!$cf_verified) {
        if (is_cf_invalid($cf)) {
            redirect(new moodle_url('/local/fsbaclogin/signup.php'));
        } else {
            $cf_added = true;
        }
        // }
...
$mform_signup = new local_fsbaclogin\signup_form(null, array('cf_added' => $cf_added, 'cf' => $cf));
