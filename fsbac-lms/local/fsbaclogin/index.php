...
$PAGE->set_url("$CFG->wwwroot/local/fsbaclogin/index.php");
...
} else if (!empty($SESSION->wantsurl) && file_exists($CFG->dirroot.'/local/fsbaclogin/weblinkauth.php')) {
    // Handles the case of another Moodle site linking into a page on this site
    // TODO: move weblink into own auth plugin
    include($CFG->dirroot.'/local/fsbaclogin/weblinkauth.php');
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    if ($user) {
        ...
        if (empty($user->confirmed)) {       // This account was never confirmed
            ...
            $resendconfirmurl = new moodle_url('/local/fsbaclogin/index.php',
                [
        ...
        if (!isguestuser() and !empty($userauth->config->expiration) and $userauth->config->expiration == 1) {
            ...
            if ($userauth->can_change_password()) {
                ...
                if (!$passwordchangeurl) {
                    $passwordchangeurl = $CFG->wwwroot.'/local/fsbaclogin/change_password.php';
                } else {
        ...
        redirect(new moodle_url(local_fsbaclogin_getloginurl(), array('testsession' => $USER->id)));
...
if (empty($SESSION->wantsurl)) {
    ...
    if ($referer &&
            $referer != $CFG->wwwroot &&
            $referer != $CFG->wwwroot . '/' &&
            $referer != $CFG->wwwroot . '/local/fsbaclogin/' &&
            strpos($referer, $CFG->wwwroot . '/local/fsbaclogin/?') !== 0 &&
            strpos($referer, $CFG->wwwroot . '/local/fsbaclogin/index.php') !== 0) { // There might be some extra params such as ?lang=.
...
if (isloggedin() and !isguestuser()) {
    ...
    $loginform = new \local_fsbaclogin\output\login($authsequence, $frm->username);
    $loginform->set_error($errormsg);
    $renderer = $PAGE->get_renderer("local_fsbaclogin");
    echo $renderer->render($loginform);
}
