...
certificate_print_text($pdf, $x, 126, 'L', '', '', 9, 'ha completato i seguenti moduli del corso');
certificate_print_text($pdf, $x, 126+6, 'L', '', '', 10, format_string($course->fullname));
$pdf->writeHTML('', true, false, false, false, '');
$hh = $pdf->GetY() + 1;
$pdf->Line($x, $hh, $x + 128, $hh, ['height' => 0.5, 'color' => array(24, 66, 135)]);

// colonna sinistra
$x = 15; $y = 134;
 certificate_print_text($pdf, $x, $y, 'L', '', '', 9, 'data');
 certificate_print_text($pdf, $x, $y+5, 'L', '', '', 10,  userdate(time(), '%d %B %Y'));
...
$x = 15; $y = 154;
certificate_print_text($pdf, $x,  $y, 'L', '', '', 9, 'numero attestato');
certificate_print_text($pdf, $x, $y+5, 'L', '', '', 10, str_pad($issue->id, 6, '0', STR_PAD_LEFT));
$pdf->Line($x,  $y+10, $x + 45, $y+10, ['height' => 0.5, 'color' => array(24, 66, 135)]);

$x = 15; $y = 174;
 certificate_print_text($pdf, $x, $y, 'L', '', '', 7, 'Il corso si è svolto online<br>sulla piattaforma LMS',50);
 certificate_print_text($pdf, $x, $y+9, 'L', '', '', 7, 'fad.fondazionescuolapatrimonio.it',45);

$x = 15; $y = 194;
certificate_print_text($pdf, $x, $y, 'L', '', '', 7, 'firmato dal Direttore<br>arch. Maria Alessandra Vittorini', 45);
...
$sql = "
select cm.id, date(from_unixtime(cmc.timemodified)) as data
from {course_modules} cm
join {course_modules_completion} cmc
on cmc.coursemoduleid=cm.id and cmc.userid=$user->id
...
if (empty($rows)) {
	    $table .= '<tr>
                  <td align="left"><font size="10">' . 'Non ci sono ancora moduli completati'. '<br></font></td>
...
foreach ($rows as $row) {
		$moduleid = $row->id;
		$cmc = $modinfo->get_cm($moduleid); // $cmc.name
	    $table .= '<tr>
                  <td align="left"><font size="10">' . $cmc->name . '<br></font></td>
