...
function subcourse_add_instance(stdClass $subcourse) {
    ...
    // Set Subcourse Name to None
    // $subcourse->name = 'None';

    if (!empty($subcourse->refcourse)) {
        // Set Subcourse Name to referred course
        if ($subcourse->name == 'ref') {
            $subcourse->name = get_course($subcourse->refcourse)->fullname;
        }
        // Set course metalink enrol method
...
function subcourse_update_instance(stdClass $subcourse) {
    ...
    if (!empty($subcourse->refcoursecurrent)) {
        // Set Subcourse Name to current referred course if user enter the word "ref". Otherwise user wll enter the name
        if ($subcourse->name == 'ref') {
            $subcourse->name = get_course($subcourse->refcoursecurrent)->fullname;
        }
        // Set course metalink enrol method
        subcourse_set_course_metalink($subcourse->course, $subcourse->refcoursecurrent);
        unset($subcourse->refcourse);
    }

    if (!empty($subcourse->refcourse)) {
        // Set Subcourse Name to referred course if user enter the word "ref". Otherwise user wll enter the name
        if ($subcourse->name == 'ref') {
            $subcourse->name = get_course($subcourse->refcourse)->fullname;
        }
        // Set course metalink enrol method
 ...
 * Set course metalink enrol method
 */
function subcourse_set_course_metalink($courseid, $refcourseid) {

    global $DB;

    // Check if instance already exists
    $instance = $DB->get_record('enrol', array('enrol' => 'meta', 'courseid' => $refcourseid, 'customint1' => $courseid), '*');
    if (!empty($instance)) {
        return;
    }
    ...
    // Get referred course
    $refcourse = get_course($refcourseid);

    // Get instance defaults
    $instance = (object) $plugin->get_instance_defaults();
    // Set instance fields
    $instance->type = 'meta';
    $instance->customint1 = $course->id;
    $instance->customint2 = 0;

    $plugin->add_instance($refcourse, (array) $instance);
}
