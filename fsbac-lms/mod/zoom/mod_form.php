...
class mod_zoom_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        if ($showschedulingprivilege || $showalternativehosts) {
            ...
            $zoomapiidentifier = get_config('zoom', 'apiidentifier');
            if ($zoomapiidentifier == "email") {
                $emails = array('sara.hassan@fondazionescuolapatrimonio.it',
                                'silvia.etzi@fondazionescuolapatrimonio.it',
                                'helpdesk@fondazionescuolapatrimonio.it',
                                'gianluca.gennai@fondazionescuolapatrimonio.it',
                                'weblearning@fondazionescuolapatrimonio.it'
                            );
                $options = array();
                foreach ($emails as $email) {
                    if (zoom_get_user($email)) {
                        $options[zoom_get_user($email)->id] = $email;
                    }
                }
                if (!empty($options)) {
                    // In fase di creazione dell'attività zoom, puoi scegliere la mail con la cui licenza creare il meeting zoom
                    if ($isnew) {
                        $noneoption = array("none" => get_string("none"));
                        $options = array_merge($noneoption, $options);
                        $mform->addElement('select', 'zoom_license', get_string('zoom_license', 'mod_zoom'), $options);
                        $mform->setDefault('zoom_license', 'none');
                        $mform->addHelpButton('zoom_license', 'zoom_license', 'mod_zoom');
                    } else {
                        $mform->addElement('static', 'zoomlicensealreadyset', get_string('zoom_license', 'mod_zoom'),
                        get_string('zoomlicensealreadyset', 'mod_zoom'));
                    }
                }
            }
