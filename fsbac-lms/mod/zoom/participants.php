...
// Loop through each user to generate id->username mapping.
$coursecontext = context_course::instance($course->id);
$enrolled = get_enrolled_users($coursecontext);
$moodleidtouids = [];
foreach ($enrolled as $user) {
    // inizio aggiunta di ARIADNE
    $moodleidtouids[$user->id] = $user->username;
...
foreach ($participants as $p) {
    ...
    if (!empty($p->userid)) {
        // inizio aggiunta di ARIADNE
        $moodleuser = $DB->get_record('user', ['id' => $p->userid], 'username, email');
        // fine aggiunta di ARIADNE
    }
    ...
    if (array_key_exists($p->userid, $moodleidtouids)) {
        ...
        // inizio aggiunta di ARIADNE
    } else if (isset($moodleuser->username)) {
        $row[] = $moodleuser->username;
        // fine aggiunta di ARIADNE
    } else {
        $row[] = '';
    }
