    ...
    public function render_activity_information(\core_course\output\activity_information $page) {
        ...
        $sql = "SELECT cm.id, m.name
                  ...
                  JOIN {modules} m
                    ON m.id = cm.module
                 WHERE cm.id = ?";
        $coursemoduledata = $DB->get_record_sql($sql, array($data->cmid));
        if ($CFG->theme == "fsbac") {
            if ($coursemoduledata->name == "zoom") {
                foreach ($data->completiondetails as $k => $activitycompletioncriterion) {
                    if ($activitycompletioncriterion->description == get_string('detail_desc:receivepassgrade', 'completion')) {
                        $data->completiondetails[$k]->description = get_string("guardazoompertemposufficiente", "format_corsofsbac");
                    }
                    if ($activitycompletioncriterion->description == get_string('detail_desc:receivegrade', 'completion')) {
                        $data->completiondetails[$k]->statuscomplete = false;
                        $data->completiondetails[$k]->statusincomplete = false;
                    }
                }
            }
        }
