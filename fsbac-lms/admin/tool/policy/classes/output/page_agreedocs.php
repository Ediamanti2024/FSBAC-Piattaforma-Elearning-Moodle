...
class page_agreedocs implements renderable, templatable {
        ...
        if ($this->isexistinguser) {
            ...
            if (!empty($this->action) && confirm_sesskey()) {
                ...
                if ((count($acceptversionids) + count($declineversionids)) != count($this->policies)) {
                    ...
                    $setnecessaryfsbacpolicycompulsory = false;
                    // controllo se il campo custom per utenti "privacy" è valorizzato a 1 per stabilire se l'utente viene dalla piattaforma vecchia
                    $privacycustomfieldid = $DB->get_field("user_info_field", "id", array("shortname" => "privacy"));
                    $privacycustomfield = $DB->get_record("user_info_data", array("fieldid" => $privacycustomfieldid, "userid" => $USER->id));
                    if ($privacycustomfield && $privacycustomfield->data == 1) {
                        // cerco i corsi a cui l'utente è enrollato con metodo manual
                        $sql = "SELECT *
                                 ...
                                 WHERE e.enrol = 'manual'
                                   AND ue.userid = ?";
                        $coursesenrolledto = $DB->get_records_sql($sql, array($USER->id));
                        if (!empty($coursesenrolledto)) {
                            foreach ($coursesenrolledto as $course) {
                                $coursecontextinstance = \context_course::instance($course->courseid);
                                $pnrrcustomfieldid = $DB->get_field("customfield_field", "id", array("shortname" => "pnrr"));
                                $pnrrcustomfield = $DB->get_record("customfield_data", array("fieldid" => $pnrrcustomfieldid, "contextid" => $coursecontextinstance->id));
                                // se esiste almeno un corso con campo custom per corsi "pnrr" non valorizzato o valorizzato a 0, rendo obbligatoria la policy NECESSARIA FSBAC
                                if (!$pnrrcustomfield) {
                                    $setnecessaryfsbacpolicycompulsory = true;
                                    break;
                                } else if ($pnrrcustomfield && $pnrrcustomfield->intvalue == 0) {
                                    $setnecessaryfsbacpolicycompulsory = true;
                                    break;
                                }
                            }
                    ...
                    if ($setnecessaryfsbacpolicycompulsory) {
                        $necessaryfsbacpolicyaccepted = false;

                        foreach ($acceptversionids as $acceptversionid) {
                            $policyversion = $DB->get_record("tool_policy_versions", array("id" => $acceptversionid));
                            if (strpos($policyversion->summary, "fsbac_necessario") !== false) {
                                $necessaryfsbacpolicyaccepted = true;
                                break;
                            }
                        }

                        if (!$necessaryfsbacpolicyaccepted) {
                            $sql = "SELECT *
                                      FROM {tool_policy_versions} tpv
                                      JOIN {tool_policy} tp
                                        ON tp.currentversionid = tpv.id";
                            $policiesversions = $DB->get_records_sql($sql);

                            $necessaryfsbacpolicyname = "";
                            foreach ($policiesversions as $policyversion) {
                                if (strpos($policyversion->summary, "fsbac_necessario") !== false) {
                                    $necessaryfsbacpolicyname = $policyversion->name;
                                    break;
                                }
                            }
                            $text = $necessaryfsbacpolicyname;
                            $message = (object) [
                                'type' => 'error',
                                'text' => get_string("mustagreethisnecessarypolicy", "tool_policy", $text)
                            ];
                        } else {
                            $message = (object) [
                                'type' => 'success',
                        ...
                        $atleastoneoptionalnecessarypolicyaccepted = false;

                        foreach ($acceptversionids as $acceptversionid) {
                            $policyversion = $DB->get_record("tool_policy_versions", array("id" => $acceptversionid));
                            if (strpos($policyversion->summary, "_necessario") !== false) {
                                $atleastoneoptionalnecessarypolicyaccepted = true;
                                break;
                            }
                        }

                        if (!$atleastoneoptionalnecessarypolicyaccepted) {
                            $sql = "SELECT *
                                    FROM {tool_policy_versions} tpv
                                    JOIN {tool_policy} tp
                                        ON tp.currentversionid = tpv.id";
                            ...
                            $necessarypoliciesnames = array();
                            foreach ($policiesversions as $policyversion) {
                                if (strpos($policyversion->summary, "_necessario") !== false) {
                                    $necessarypoliciesnames[] = $policyversion->name;
                                }
                            }
                            $text = implode(", ", $necessarypoliciesnames);
                            $message = (object) [
                                'type' => 'error',
                                'text' => get_string("mustagreeatleastoneoptionalpolicy", "tool_policy", $text)
                            ];
                        } else {
                            $message = (object) [
                                'type' => 'success',
    ...
    protected function redirect_to_previous_url() {
        ...
        if ($this->isexistinguser) {
            ...
            if ($issignup) {
                ...
                $returnurl = new moodle_url('/local/fsbaclogin/signup_with_cf.php');
    ...
    public function export_for_template(renderer_base $output) {
        ...
        // inizio aggiunta di ARIADNE
        foreach ($data->policies as $k => $policy) {
            $policyversion = $DB->get_record("tool_policy_versions", array("id" => $policy->id));
            if (strpos($policyversion->summary, "dicolab_") !== false) {
                $data->policies[$k]->dicolab = true;
            }
        }

        usort($data->policies, function ($x, $y) {
            if (strpos($x->summary, "fsbac_") !== false && strpos($y->summary, "fsbac_") !== false) {
                return strcasecmp($x->name, $y->name);
            } else if (strpos($x->summary, "fsbac_") !== false && strpos($y->summary, "fsbac_") === false) {
                return -1;
            } else if (strpos($x->summary, "fsbac_") === false && strpos($y->summary, "fsbac_") !== false) {
                return 1;
            } else {
