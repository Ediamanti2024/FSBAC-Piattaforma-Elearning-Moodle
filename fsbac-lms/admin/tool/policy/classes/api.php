...
class api {
    ...
    public static function list_current_versions($audience = null) {
        ...
        foreach (static::list_policies() as $policy) {
            ...
            if ($audience && !in_array($policy->currentversion->audience, [policy_version::AUDIENCE_ALL, $audience])) {
                ...
                if ($USER && $USER->id == 0) {
                    if ($policy->currentversion->audience == 1) {
                        continue;
                    }
                } else {
                    if ($policy->currentversion->audience == 0) {
                        continue;
                    }
                }
    ...
    public static function update_policyagreed($user = null) {
        ...
        foreach ($DB->get_records_sql($sql, $params) as $policyacceptance) {
            ...
            if ($policyacceptance->optional == policy_version::AGREEMENT_OPTIONAL && $policyacceptance->status !== null && $policyacceptance->status == 1) {
                $sql = "SELECT *
                          FROM {tool_policy_versions} tpv
                          JOIN {tool_policy} tp
                            ON tp.currentversionid = tpv.id
                         WHERE tp.id = ?";
                $policyversion = $DB->get_record_sql($sql, array($policyacceptance->id));
        ...
        $policyagreed = $allresponded && $atleastoneoptionalnecessarypolicyaccepted;
